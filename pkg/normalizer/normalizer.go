...
func MetaFromOtelProfile(p *pprofextended.Profile, name string, sampleIndex int, duration int64) profile.Meta {
	periodType := profile.ValueType{}
	if p.PeriodType != nil {
		periodType = profile.ValueType{Type: p.StringTable[p.PeriodType.TypeStrindex], Unit: p.StringTable[p.PeriodType.UnitStrindex]}
	}

	sampleType := profile.ValueType{}
	if p.SampleType != nil {
		sampleType = profile.ValueType{Type: p.StringTable[p.SampleType[sampleIndex].TypeStrindex], Unit: p.StringTable[p.SampleType[sampleIndex].UnitStrindex]}
	}
